...
export const useChartOptions = (
    ...
    chart: {
      ...
      events: {
        legendClick: function(_chartContext, seriesIndex, opts) {
          ...
          const seriesName = seriesNames && seriesIndex !== undefined && seriesIndex >= 0 && 
                           seriesIndex < seriesNames.length ? seriesNames[seriesIndex] : '';
    ...
    tooltip: {
      custom: function({ seriesIndex, dataPointIndex }) {
        ...
        if (auditDetails.length > 0) {
          auditInfoHtml = auditDetails.map((detail: any) => {
            const scoreInfo = detail.score !== undefined ? `<br/>Score: ${detail.score}` : '';
        ...
        let statusColor = '';
        switch(status) {
          case 'visited': statusColor = colors.visited; break;
          case 'skipped': statusColor = colors.skipped; break;
          case 'excluded': statusColor = colors.excluded; break;
          default: statusColor = colors.unknown;
        }
  ...
  }), [title, dates, floors, data, auditInfo, isDarkMode, yAxisOffsetX, leftPadding]);
};