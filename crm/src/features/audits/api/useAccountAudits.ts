...
import { AuditWithFloors, AccountFloorConfig } from "../types";
 ...
 */
export const fetchAccountAudits = async (
  accountId: string,
): Promise<AuditWithFloors[]> => {
  try {
 ...
 */
export const getAccountFloorConfig = (
  account: RecordModel,
): AccountFloorConfig => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        if (
          account.floors_min !== undefined &&
          account.floors_max !== undefined
        ) {
