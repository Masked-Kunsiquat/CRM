...
const createOrganization = async (data: CreateOrganizationData): Promise<RecordModel> => {
  ...
  let addressId = "";
  if (data.address) {
    const [street, city, state, zip_code] = data.address.split(",").map(part => part.trim());
  ...
  formData.append("is_home_company", data.is_home_company?.toString() || "false");
...
const updateOrganization = async (id: string, data: UpdateOrganizationData): Promise<RecordModel> => {
  ...
  if (data.is_home_company !== undefined) formData.append("is_home_company", data.is_home_company.toString());
